...
(function() {
  ...
  async function waitForElements() {
    return new Promise((resolve) => {
      ...
      const checkElements = () => {
        attempts++;
        const cards = document.querySelectorAll('.product-card__product, .search-result, [class*="product"], [class*="listing"]');
        ...
        if (cards.length > 0) {
          ...
          const hasContent = Array.from(cards).some(card => {
            const title = card.querySelector('.product-card__title, .search-result__title, [class*="title"], [class*="name"]');
  ...
  async function cardSearch(params) {
    try {
      ...
      // Get all product cards using multiple selectors
      const productCards = Array.from(document.querySelectorAll(
        '.product-card__product, .search-result, [class*="product"], [class*="listing"]'
      ));
      ...
      const validProducts = productCards.map((card, index) => {
        ...
        const titleElement = card.querySelector(
          '.product-card__title, .search-result__title, [class*="title"], [class*="name"]'
        );
        const priceElement = card.querySelector(
          '.inventory__price-with-shipping, [class*="price"]'
        );
  ...
  // Return the card search function
  return cardSearch;
})();
