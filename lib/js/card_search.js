...
async function waitForElements() {
  return new Promise((resolve) => {
    ...
    const maxAttempts = isProduction ? 400 : 100; // 40 seconds in production, 10 seconds locally
    const stabilityWait = isProduction ? 20000 : 5000; // 20 seconds in production, 5 seconds locally
    ...
    const checkElements = () => {
      ...
      if (cards.length > 0) {
        ...
        if (hasContent) {
          ...
          }, stabilityWait); // Use environment-specific wait time
          return;
        }
      }
